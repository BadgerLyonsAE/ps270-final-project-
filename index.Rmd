...
```{r}
Pivot_Sample |>
  filter(Election_year=="Obama MOV 2008") |>
  ggplot(aes(x = MOV)) + geom_histogram(binwidth = 2)

Pivot_Sample |>
  filter(Election_year=="Obama MOV 2012") |>
  ggplot(aes(x = MOV)) + geom_histogram(binwidth = 2)

Pivot_Sample |>
  filter(Election_year=="Trump MOV 2016") |>
  ggplot(aes(x = MOV)) + geom_histogram(binwidth = 2)

Pivot_Sample |>
  filter(Election_year== "MOV 2020") |>
  ggplot(aes(x = MOV)) + geom_histogram(binwidth = 2)


ggplot(data = Pivot_Sample, aes(x = MOV)) + geom_histogram(binwidth = 2) + facet_wrap(~Election_year)

```


```{r}
mutate(Biden MOV 2020 = case_when(status == "boomerang" ~ MOV 2020, status == "retained" ~ -MOV 2020))
```
...
- I don't know where the code for labeling the x axis line is coming from so i was not able to change it, somehow I need to make it so the collums are only for each year not showing the MOV for all three candiates for the same year every column (obviosly no data for obama MOV in 2020, etc)
...
## concluson 
say my theory fgarbage but now I amthinking it is more important that it hurts the party in charge when there is high enemplouyement
or change hypthothesis that, high un-employment affects the incumbent party, 
...
regressoin 
  - lm(MOV ~ unemployment + status(boomerang or retained + status * unemployment ))
  tells us what is the effect of unemployment on all of the margin of victories for all of these counties, if I get a small p value from the combined ones it means there is a difference in effects of unemplpuyment on return and boomerang counties 